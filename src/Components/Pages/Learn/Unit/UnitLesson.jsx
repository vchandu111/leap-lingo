...
import Question from "../../Lessons/Question";
import FIBComponent from "../../Lessons/FIBComponent";
...
import correctSound from "../../../../assets/images/audio/success.mp3";
import wrongSound from "../../../../assets/images/audio/wrong.mp3";
import LessonComplete from "../../Lessons/LessonComplete";
...
const UnitLesson
 = ({ questions }) => {
  ...
  const wrongSoundRef = useRef(new Audio(wrongSound));
  ...
  useEffect(() => {
    // Initialize audio with the imported files
  ...
  const handleContinue = () => {
    ...
    if (currentQuestionIndex < questions.length - 1) {
      ...
      const timeSpent = formatTimeSpent(Date.now() - startTimeRef.current);
  ...
  const handleHeartRefill = () => {
    ...
    setIsCorrect(false);
    setShowStatus(false);
    setCurrentQuestionIndex(0); // Optionally restart from beginning
  };
  ...
  return (
    <div className="flex flex-col items-center justify-between h-full min-h-screen bg-gray-50 text-gray-800 p-4">
      {showComplete ? (
        <LessonComplete stats={stats} />
      ) : (
        <>
          <ToastContainer />

          {showHeartModal && <HeartModal onRefill={handleHeartRefill} />}
          ...
          <div className="w-1/2 flex justify-between mt-8 mb-4 px-4">
            {showStatus ? (
              <button
                className={`flex items-center justify-center px-8 py-3 rounded-xl ${
                  isCorrect
