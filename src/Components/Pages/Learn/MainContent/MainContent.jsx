    ...
    <div className="space-y-8">
      {units.map((unit, index) => (
        <div key={unit.id} className="space-y-4">
          <div
            className={`${unit.color} rounded-xl p-6 flex justify-between hover:shadow-lg transition-all duration-200`}
          >
            <div>
            ...
            <div
              onClick={() => navigate(`/learn/guidebook/${unit.id}`)}
          ...
          {index < units.length - 1 && (
            <div className="flex flex-col items-center gap-4 py-2">
              <div
                onClick={() => navigate(`/learn/${unit.id}/practice`)}
                className="relative w-12 h-12 rounded-full bg-green-100 flex items-center justify-center ml-12 mt-20 cursor-pointer group"
              >
